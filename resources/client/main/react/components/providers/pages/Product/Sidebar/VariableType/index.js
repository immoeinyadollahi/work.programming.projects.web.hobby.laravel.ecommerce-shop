...
export default function VariableType({ product }) {
  ...
  const initialSelectedAttributes = useMemo(() => variations.find((variation) => variation.id === product.selected_su.id).variable_product_type_attributes.reduce((acc, current) => ((acc[current.pivot.attribute_id] = current.id), acc), {}), []);
  ...
  const selectedVariation = useMemo(() => variations.find((variation) => variation.variable_product_type_attributes.every((variationAttribute) => selectedAttributes[variationAttribute.pivot.attribute_id] === variationAttribute.id)), [selectedAttributes]);
    ...
    const attributes = productAttributes.map((attribute) => {
      const filteredVariations = variations.filter((variation) =>
        Object.entries(previousAttributes)
          ...
          .every(([previousAttributeId, previousAttributeValueId]) => variation.variable_product_type_attributes.some((variationAttribute) => variationAttribute.pivot.attribute_id === previousAttributeId && variationAttribute.id === previousAttributeValueId))
      );
      const attributeValues = _.uniqBy(
        filteredVariations.map((variation) => variation.variable_product_type_attributes.find((variationAttribute) => variationAttribute.pivot.attribute_id === attribute.id)),
        "id"
      );
