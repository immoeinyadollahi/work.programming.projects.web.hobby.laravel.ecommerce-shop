        ...
        <div className="product-card__actions">
          <div className="product-card__prices">
            {isInStock ? (
              selectedSu.discounted_price ? (
                <>
                  <span className="product-card__new-price">{`${_String.getThousandSeparated(selectedSu.discounted_price)} تومان`}</span>
                  <span className="product-card__old-price mr-1">{`${_String.getThousandSeparated(selectedSu.price)} تومان`}</span>
                </>
              ) : (
          ...
          <div className="product-card__buttons">
            <div className="d-flex justify-content-end m-0 w-100">
              <a href={`/favourite/product/${product.id}`} className="btn btn-light btn-svg-icon btn-svg-icon--fake-svg product-card__wishlist" style={{ fill: product.is_user_favourite ? "red" : "" }}>
                <svg width="16px" height="16px">
                  <use xlinkHref="/public/main/images/sprite.svg#wishlist-16"></use>
                </svg>
                <span className="fake-svg-icon fake-svg-icon--wishlist-16" />
              </a>
              <a href={`/compare/${product.id}`} className="btn btn-light btn-svg-icon btn-svg-icon--fake-svg product-card__compare">
                <svg width="16px" height="16px">
