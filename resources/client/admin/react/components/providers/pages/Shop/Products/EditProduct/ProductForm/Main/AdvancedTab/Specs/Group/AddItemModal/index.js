...
export default function AddItemModal({ group, onClose, onSubmit }) {
  ...
  const formik = useFormik({
    ...
    async onSubmit({ name, type, text_value, boolean_value }) {
      ...
      try {
        const {
          data: { item },
        } = await specificationsHttp.postGroupItem(group.id, {
          name,
          type,
  ...
  return (
    <Modal show={isModalActive} onHide={() => setIsModalActive(false)} onExited={onClose} centered>
      <Modal.Header closeButton>
        <Modal.Title>افزودن آیتم</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <form>
          <div className="mb-5">
            <label className="form-label required">نام</label>
            <input type="text" className={`form-control${errors.name ? " is-invalid" : ""}`} name="name" placeholder="نام آیتم را وارد کنید" value={formik.values.name} onChange={formik.handleChange} />
