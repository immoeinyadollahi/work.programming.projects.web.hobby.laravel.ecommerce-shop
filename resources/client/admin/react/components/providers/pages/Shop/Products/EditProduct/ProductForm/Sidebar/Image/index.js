...
export default function Image() {
  ...
  const handleImageInputChange = (e) => {
    ...
    if (file.size > 5000000) {
      ...
      reader.addEventListener("load", (e) => updateProductState((state) => (state.image ? (state.image.url = e.target.result) : (state.image = { url: e.target.result }))));
  ...
  const handleStartUpload = async () => {
    try {
      ...
      updateProductState((state) => (state.image = { id: image_id, url: image_url }));
      resetUpload();
    } catch (err) {
      handleHttpError(err, {
        ...
        <div className="image-input image-input-outline" data-kt-image-input="true">
          ...
          <div
            ...
            style={
              productState.image
                ? {
                    backgroundImage: `url(${productState.image.url})`,
          ...
          {/*begin::انصراف*/}
          {productState.image && (
            <span className="btn btn-icon btn-circle btn-active-color-primary w-25px h-25px bg-body shadow" data-kt-image-input-action="remove" onClick={() => (updateProductState((state) => (state.image = null)), uploadFile && setUploadFile())}>
