...
export default function DeleteConfirmModal({ onRequest, title, onClose, onConfirm }) {
    ...
    <Modal show={isModalActive} onHide={() => setIsModalActive(false)} onExited={onClose} centered>
      ...
      <Modal.Footer>
        <button className="btn btn-danger" onClick={handleConfirm} disabled={isDeleting}>
          {isDeleting ? <span className="spinner-border spinner-border-sm"></span> : "مطمئنم"}
        </button>
        <button className="btn btn-secondary" onClick={() => setIsModalActive(false)} disabled={isDeleting}>
          بستن
        </button>
      </Modal.Footer>
    </Modal>
